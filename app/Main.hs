...
main :: IO ()
main = do
    smartNet <- train quadraticLoss 1 examples (Left $ StopCriteria 0.00000072 60000) <$> initNet testNet
    print $ (unTotErrF $ getTotalLoss quadraticLoss) (map _output examples) $ map (runNetwork smartNet . _input) examples
    putStrLn ""
